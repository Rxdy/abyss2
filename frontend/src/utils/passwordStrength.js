...
function alphabetSize(password) {
...
function usefulLength(password) {
  ...
  chars.forEach((char, i) => {
    ...
    if (previous === undefined) return void count++
...
function adviceFor({ password, bits, alphabet, commonWords, useful, acceptable }) {
  ...
  const mixed = alphabet > 36
    ? ''
    : ' Ou mélangez majuscules, minuscules, chiffres et symboles.'
 ...
 *
 * @returns {{
 *   bits: number,            entropie estimée
 *   percent: number,         0 à 100, part de TARGET_BITS atteinte
 *   level: number | null,    index dans STRENGTH_LEVELS (null : champ vide)
 *   label: string,           libellé du niveau ('' : champ vide)
 *   acceptable: boolean,     longueur permise ET niveau requis atteint
 *   hint: string,            conseil ('' si rien à dire)
 * }}
 */
